...
class PokemonBot:
    def __init__(self):
        ...
        self.all_pokemon_names = [
            ...
            "dragonair", "squirtle", "wartortle","charmeleon", "bulbasaur", "ivysaur","raticate","ninetales","arcanine","ponyta","rapidash","muk","koffing","weezing","magmar"
        ]
        ...
        self.max_encounter_time = 30           # Max time for one encounter (seconds)
        ...
        self.max_rest_duration = 20.0  # Maximum rest time in seconds
    ...
    def detect_any_pokemon(self, detected_text):
        ...
        for noise in noise_patterns:
            if noise in cleaned_text or cleaned_text == noise:
                logger.debug(f"Detected OCR noise, ignoring: '{cleaned_text}'")
                return "none", None
        ...
        logger.debug(f"Detected text ignored as noise: '{cleaned_text}'")
        return "none", None
    ...
    def press_key_windows_api(self, key):
        ...
        logger.debug(f"Pressed key: {key} (duration: {press_duration:.3f}s)")
        return True
    ...
    def hold_key_down(self, key):
        ...
        logger.debug(f"Holding down key: {key}")
        return True
            ...
            if self.use_long_press and self.continuous_movement:
                ...
                if not self.currently_pressed_key:
                    if self.hold_key_down(current_key):
                        ...
                        logger.debug(f"Starting continuous movement: {current_key} (duration: {self.long_press_duration:.3f}s)")
                    else:
                ...
                # Check if it's time to switch direction
                    ...
                    if self.hold_key_down(new_key):  # Immediately press new key
                        self.movement_start_time = current_time
                        logger.debug(f"Switching movement: {old_key} → {new_key}")
                    else:
                        # Fallback to pyautogui
                ...
                if not self.currently_pressed_key or self.currently_pressed_key != current_key:
                    if self.hold_key_down(current_key):
                        logger.debug(f"Long pressing: {current_key} for {self.long_press_duration}s")
                    else:
                        # Fallback to pyautogui
                        pyautogui.keyDown(current_key)
                        self.currently_pressed_key = current_key
                ...
                if self.steps_in_direction >= self.max_steps_per_direction:
                    ...
                    logger.debug(f"Changing direction to: {self.movement_keys[self.current_direction]}")
                
            else:
                # Normal single key press
                if self.press_key_windows_api(current_key):
                    logger.debug(f"Moving: {current_key}")
                else:
    ...
    def start_bot(self, duration_seconds=60):
        ...
        try:
            while time.time() - start_time < duration_seconds:
                ...
                if detection_result:
                    ...
                    logger.debug("No target Pokemon found, moving to search...")
                    self.move_character()
        ...
        logger.info(f"Bot session ended. Total catches: {catches}, Total cycles: {cycles}")
...
def main():
    ...
    try:
        ...
        bot.start_bot(duration_seconds=1200)  # Run for 20 minutes initially
    except KeyboardInterrupt:
